...
static void moveLeftUpIfPivotBigger(ListElement* array, int*left, int right,
		ListElement pivot, CompareListElements compareFunction);
static void moveRightDownIfPivotSmaller(ListElement* array, int left,
		int* right, ListElement pivot, CompareListElements compareFunction);
static void quick_sort(ListElement array[], int n,
...
ListResult listSort(List list, CompareListElements compareElement) {
	...
	ListElement* elements_array = malloc(size * sizeof(ListElement));
	ListResult result = listNodeToArray(list, elements_array, size);
	if (elements_array == NULL || result != LIST_SUCCESS) {
		free(elements_array);
		return LIST_OUT_OF_MEMORY;
	}
	quick_sort(elements_array, size, compareElement);
...
static void moveLeftUpIfPivotBigger(ListElement* array, int*left, int right,
		ListElement pivot, CompareListElements compareFunction) {
	while ((*left) <= right) {
		int result = (compareFunction(array[(*left)], pivot));
		if (result <= 0) {
			(*left)++;
		} else {
			break;
...
static void moveRightDownIfPivotSmaller(ListElement* array, int left,
		int* right, ListElement pivot, CompareListElements compareFunction) {
	while (left <= (*right)) {
		int result = (compareFunction(array[(*right)], pivot));
		if (result > 0) {
			(*right)--;
		} else {
			break;
...
static int partition(ListElement* array, int length,
	...
	while (left <= right) {
		moveLeftUpIfPivotBigger(array, &left, right, pivot, compareFunction);
		moveRightDownIfPivotSmaller(array, left, &right, pivot,
				compareFunction);
		if (left < right) {
...
static void quick_sort(ListElement array[], int n,
	...
	swap(array, 0, t);
	quick_sort(array, t, compareFunction);
	quick_sort(array + t + 1, n - t - 1, compareFunction);
}
